...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run live capital ramp validation demo.")

    parser.add_argument("--session-name", default="live_micro_demo")
    parser.add_argument("--export", action="store_true")
    parser.add_argument("--output-dir", default="artifacts/live")
    parser.add_argument("--name", default="live_capital_ramp_validation_demo")
...
def main() -> int:
    ...
    events = build_demo_live_session_events(args.session_name)
    performance = build_live_performance_report(events=events, session_name=args.session_name)
    risk_audit = build_live_risk_audit_report(events=events, session_name=args.session_name)
    ...
    if args.export:
        ...
        print(f"Live capital ramp validation exported: {path}")
